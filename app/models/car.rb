...
class Car < ApplicationRecord
  ...
  def self.sql_search(search, city, brand, model, registration_year, variant, registration_state, kilometer_driven)
    @cars = Car.where("upper(brand) like upper('%#{search}%') or upper(model) like upper('%#{search}%') or upper(city) like upper('%#{search}%') or upper(variant) like upper('%#{search}%') or upper(state) like upper('%#{search}%')")
  end
